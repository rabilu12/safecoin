...
def send_mail(to, template, context):
    html_content = render_to_string(f'user/emails/{template}.html', context)
    text_content = render_to_string(f'user/emails/{template}.txt', context)
...
def send_activation_change_email(request, email, code):
    context = {
        ...
        'uri': request.build_absolute_uri(reverse('user:change_email_activation', kwargs={'code': code})),
    }
...
def send_reset_password_email(request, email, token, uid):
    context = {
        ...
        'uri': request.build_absolute_uri(
            reverse('user:restore_password_confirm', kwargs={'uidb64': uid, 'token': token})),
    }
